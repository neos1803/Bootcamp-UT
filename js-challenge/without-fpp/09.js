...
class Purchase{
    ...
    purchaseMadeByPerson(person) {
        let sum = 0;
        this.data.forEach((data) => {
            if (data.customer.name.toLowerCase() === person.toLowerCase()) {
    ...
    priceByLimit(limit) {
        ...
        this.data.forEach((data) => {
            ...
            if (sum > limit) name.push(data.customer.name);
        });
        console.log(name);
    }
}

let newPurchase = new Purchase(obj)
newPurchase.byMonth("02");
newPurchase.purchaseMadeByPerson("Ari");
newPurchase.priceByLimit(300000);